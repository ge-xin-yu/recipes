...
def log(mesg):
    with open('d://验证日志.txt', 'a') as f:
        f.write(mesg+'\n')
            

if __name__ == "__main__" :   
    #向日志文件输出验证日期
    op_date = date.isoformat(date.today())
    log('#####{}#####'.format(op_date))
    rounds = 0
    #登录地址
    ...
    if 'sso_hash' and 'sso_hash_low' in r.cookies.keys():
        for _ in range(20):        
            ...
            #某些情况下，网站出现问题导致无法获取有效次数，此时函数返回None。
            if counts:                
                mesg = '第{}轮验证：有效次数 {} 次'.format(rounds,counts)
                print(mesg+'\n')
                #记录日志
                log(mesg)
                if counts >= 30:
